...
rm(area)		# remove object (put the name where "object" is now)
...
acs = read.csv(text=readLines("./ACS_17_5YR_B25077_with_ann.csv")[(-2)])
##acs_subset = acs[colSums(!is.na(acs)) > 0] ## clean up NA's by columns 
View(acs)
...
dof$hoval16 = acs$ACS17_5yr_B25077001[match(dof$id, acs$GEO.id2)]
dof$emp17 = bls$emp_jun17[match(dof$id, bls$GEO.id2)]   # same thing for bls data 
dof$emp15 = bls$emp_jun15[match(dof$id, bls$GEO.id2)]   # one time for each row (bonus: can you turn this into a loop???)
...
plot(dof$hoval16, log(dof$pop17))
